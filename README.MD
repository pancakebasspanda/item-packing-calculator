...
`DELETE` to remove a pack size from the system at runtime. (Returns a 204 No Content status code with no body)

```shell
curl -i -X DELETE http://localhost:8080/api/v1/packs \
  -H "Content-Type: application/json" \
  -d '{"size": 750}'
```
...
```shell
curl -i -X POST http://localhost:8080/api/v1/packing/calculate \
  -H "Content-Type: application/json" \
  -d '{"orderQuantity": -500}'
```
...
Date: Sun, 16 Aug 2026 19:01:24 GMT
...
curl -i -X POST http://localhost:8080/api/v1/packing/calculate \
  -H "Content-Type: application/json" \
  -d '{"orderQuantity": hello}'
```

```shell
HTTP/1.1 400 Bad Request
Date: Sun, 16 Aug 2026 19:01:41 GMT
